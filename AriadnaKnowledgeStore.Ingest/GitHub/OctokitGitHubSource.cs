...
namespace AriadnaKnowledgeStore.Ingest.GitHub;
...
public sealed class OctokitGitHubSource : IGitHubSource
{
    ...
    {
        _client = new GitHubClient(new ProductHeaderValue("AriadnaKnowledgeStore-mvp"))
        {
    ...
    {
        // Directorio temporal
        var root = Path.Combine(Path.GetTempPath(), "AriadnaKnowledgeStore", Guid.NewGuid().ToString("N"));
